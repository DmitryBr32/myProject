...
export default function MoodboardPage({ user }) {
  ...
  return (
    <div>
      <Wrap spacing='30px' templateColumns="repeat(4, 1fr)" gap={6} m={10}>
        {user[1] ? (
        ...
        {boards.map((board) => (
          <OneBoard key={board.id} prop={[board.id, board.title, user[1]]} board={board} setBoards={setBoards} setClick={setClick} />
        ))}
      </Wrap>
    </div>
  );
